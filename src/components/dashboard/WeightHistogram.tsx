...
function createHistogramData(data: number[], binSize: number = 0.05) {
  if (data.length === 0) return [];
  
  // Fixed range from 0.05kg (50g) to 0.11kg (110g) to match the reference image
  const min = 0.05;
  const max = 0.11;
...
function calculateStatsWithBins(data: number[], bins: any[]) {
  ...
  return {
    mean: Math.round(mean * 100) / 100, min, max, range, q1, q3,
...
export function WeightHistogram() {
  ...
  const histogramData = createHistogramData(weightData, 0.05); // Use 0.05kg (50g) bin size
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <Tooltip>
                ...
                <TooltipContent>
                  <p className="max-w-xs">Histogram showing the frequency distribution of fish weights in the population</p>
                </TooltipContent>
              </Tooltip>
            </div>
          </div>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <ComposedChart data={histogramData} margin={{ top: 20, right: 30, left: 20, bottom: 40 }} barCategoryGap={0}>
                ...
                <XAxis 
                  ...
                  label={{ value: 'Weight (kg)', position: 'insideBottom', offset: -10, style: { textAnchor: 'middle', fill: "hsl(var(--muted-foreground))" } }}
                />
                <YAxis
                  axisLine={true}
                  tickLine={true}
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="space-y-1">
                <div className="flex justify-between">
                  ...
                  <span className="font-medium text-foreground">{stats.mean}kg</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Q1:</span>
                  <span className="font-medium text-foreground">{stats.q1}kg</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Q3:</span>
                  <span className="font-medium text-foreground">{stats.q3}kg</span>
                </div>
              </div>
              <div className="space-y-1">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Min:</span>
                  <span className="font-medium text-foreground">{stats.min}kg</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Max:</span>
                  <span className="font-medium text-foreground">{stats.max}kg</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground">Range:</span>
                  <span className="font-medium text-foreground">{stats.range}kg</span>
                </div>
              </div>
            </div>
          </div>
